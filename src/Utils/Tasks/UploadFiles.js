...
const { UPLOAD_CACHE_PATH } = require("../Constants");
...
module.exports = async function UploadAssets() {
	...
	for (const asset of UploadList) {
		const filePath = `${UPLOAD_CACHE_PATH}/${asset.discord_id}.${asset.extension}`;
		if (!fs.existsSync(filePath)) {
			// something went wrong, we know the asset exists, but we don't have the data
	...
	const files = fs.readdirSync(UPLOAD_CACHE_PATH);
	for (const file of files) {
		...
		if (asset) {
			...
			await fs.promises.unlink(`${UPLOAD_CACHE_PATH}/${file}`);
			continue;
		}

...
module.exports = {
	...
	execute: async function(interaction, client, args) {
		...
		const guilds = await connection.query(`SELECT COUNT(*) as count FROM Guilds`).then(res => res[0].count);
		...
		const messages = await connection.query(`SELECT COUNT(*) as count FROM Messages`).then(res => res[0].count);
